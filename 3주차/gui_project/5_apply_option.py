...
def add_file():
    files = filedialog.askopenfilenames(title='이미지 파일을 선택하세요.', \
        ...
        initialdir =  r"C:\Users\pc\나도코딩\2주차\pygame_project\images") # 최초 C드라이브로 뜸
...
def merge_image():
    try:
        ...
        # 포맷
        img_format = cmb_format.get().lower() # png, jpg bmp 값을 받아와서 소문자로변경
...
btn_desdt_path = Button(path_frame, text ='찾아보기',width=10, command=browse_dest_path)
btn_desdt_path.pack(side='right', padx = 5, pady =5)
...
root.resizable(False, False) # x(너비), y(높이) 값 변경 불가 (창크기 변경 불가), default는 가능함
root.mainloop()